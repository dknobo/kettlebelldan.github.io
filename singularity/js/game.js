(() => {
  ...
  const NAMES = ["Claude", "Grok", "ChatGPT", "DeepSeek"];
  const LOGO_SRC = [
    "logos/claude.png",
    "logos/grok.png",
    "logos/chatgpt.png",
  ...
  function draw() {
    ...
    const logoSize = Math.max(22, Math.min(world.cellW, world.cellH) * 2.7);
    for (const ball of world.balls) {
      ...
      if (speedMul > 1.15) {
        ...
        ctx.lineTo(
          ball.x - (ball.vx / spd) * logoSize * (0.55 + speedMul * 0.12),
          ball.y - (ball.vy / spd) * logoSize * (0.55 + speedMul * 0.12)
        );
        ctx.stroke();
        ctx.globalAlpha = 1;
      }
  ...
  function loadLogos() {
    return Promise.all(LOGO_SRC.map((src, i) => new Promise((resolve) => {
      const img = new Image();
      img.onload = () => { LOGOS[i] = img; resolve(); };
      img.onerror = () => resolve();
      img.src = src;
    })));
  }
