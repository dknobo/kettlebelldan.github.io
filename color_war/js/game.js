(() => {
  ...
  const PLAYERS = 12;
  const MAX_BALLS_PER_TEAM = 27;
  const MAX_TOTAL_BALLS = 180;
  ...
  const PALETTE = [
    "#0f4f58", "#1a3a78", "#541e6a", "#6e1840",
    "#0f5240", "#1a4f86", "#382474", "#6e241c",
    "#0f4a5c", "#243468", "#5c184c", "#145246",
  ];

  const ACCENT = [
    "#3ec8c0", "#4a7dff", "#b25cff", "#e04670",
    "#2ecf8a", "#5aa6ff", "#7a62ff", "#e05548",
    "#2ab8d4", "#6d7dff", "#d14ca8", "#2cb892",
  ];
  ...
  function resetWorld(seed) {
    ...
    const seeds = makeSeeds(PLAYERS, cols, rows, random);
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        for (let i = 0; i < seeds.length; i++) {
          ...
          const d = dx * dx + dy * dy;
          if (d < nearest) { nearest = d; owner = i; }
        }
    ...
    tileFill = PALETTE.map((c) => mixHex(c, "#050508", 0.08));
